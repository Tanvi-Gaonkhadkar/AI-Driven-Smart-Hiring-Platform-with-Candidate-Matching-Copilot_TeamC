...
def get_employee(employee_name):
    ...
    # Performance Trend
    try:
        perf = json.loads(employee["performance_trend"] or "[]")
    except:
        perf = []

    if len(perf) != 6:
        perf = [75, 78, 80, 82, 85, 88]

    employee["performance_trend"] = perf


    # Attendance Trend
    try:
        att = json.loads(employee["attendance_trend"] or "[]")
    except:
        att = []

    if len(att) != 6:
        att = [92, 94, 95, 96, 97, 98]

    employee["attendance_trend"] = att
...
def candidate_exists(candidate_name):
    conn = get_connection()
...
def get_ai_summary(
    ...
    return df
def bulk_update_candidate_status(candidate_names, status):
